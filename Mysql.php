 ...
 * исключение, а пытается преобразовать аргумент к нужному типу заполнителя в соответствии с правилами преобразования
 * типов в PHP.
 *
 ...
 *      Данный заполнитель предназначен для случаев, когда имя таблицы или поля передается в запроос через переменную:
 ...
 *
 * ?p - заполнитель числа с плавающей точкой.
 *      В режиме MODE_TRANSFORM любые скалярные данные принудительно приводятся к типу float
...
class Krugozor_Database_Mysql
{
    ...
    public function setTypeMode($value)
    {
        ...
        {
            throw new Krugozor_Database_Mysql_Exception('Указан неизвестный тип режима');
        }
