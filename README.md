...
$db->query('SELECTFROM table WHERE field = ?i', 'вася'); - в данном случае будет выброшено исключение
...
$db->query('SELECTFROM table WHERE field = "?s"', 123); - будет выброшено исключение
...
$db->query('SELECTFROM table WHERE field IN (?as)', array(null, 123, true, 'string')); - будет выброшено исключение
...
Пример выражения:
    $db->query('SELECTFROM table WHERE f1 = "?s", f2 = "?s", f3 = "?s"', null, 123, true);
Результат преобразования:
    SELECTFROM table WHERE f1 = "", f2 = "123", f3 = "1"
...
Пример выражения:
    $db->query('SELECTFROM table WHERE f1 = ?i, f2 = ?i, f3 = ?i, f4 = ?i', null, '123abc', 'abc', true);
Результат преобразования:
    SELECTFROM table WHERE f1 = 0, f2 = 123, f3 = 0, f4 = 1
...
перечисления всегда используются в запросах, где наличие кавчек обязательно или не играет роли:
   ...
   $db->query('SELECTFROM table WHERE field IN (?as)', array('55', '12', '132'));
   -> SELECTFROM table WHERE field IN ("55", "12", "132")